...
# ╔═╡ 4f1c4c58-aee5-4b08-89fb-d7f58840e53c
using Plots
...
# ╔═╡ 3bbd0865-a262-4f15-818e-13895a0692c1
rand(n)

# ╔═╡ 0825bc69-b519-4068-a17e-20abd4fc9dae
rand(Int, n)

# ╔═╡ 62113262-0998-4e95-bed9-326e2c08c685
rand(Bool, n)
...
function aleatorioUniforme(a, b)
	...
	# Toma dos argumentos a y b, que representan los extremos del intervalo. Luego, utiliza la función rand() para generar un número aleatorio en el rango [0, 1) y lo escala al rango [a, b] mediante la fórmula a + (b - a) * rand()
    return a + (b - a) * rand()
end

# ╔═╡ b7505842-340b-41c2-aec6-25d6378f84f6
begin
	aleatorioUniforme(0, 10)  # Ejemplo: Generar un número aleatorio entre 0 y 10
end
...
begin
function verificarDistribucion(a, b, num_samples)
    # Generar una lista de números aleatorios usando la función aleatorioUniforme
    samples = [aleatorioUniforme(a, b) for _ in 1:num_samples]
    
    # Crear el histograma de los valores generados
    histogram(samples, bins = 20, legend = false,
              xlabel = "Valores", ylabel = "Frecuencia",
              title = "Distribución de valores generados")
end
end

# ╔═╡ f6b5aea9-45ba-4ad2-bae6-3e14b4b8ef72
# Verificar la distribución en el intervalo [0, 10] con 1000 muestras
verificarDistribucion(0, 10, 1000)
...
# ╔═╡ bfffc99d-5335-4c00-90a8-39490400a364
# La funcion utiliza rand(Bool) para generar aleatoriamente un booleano (true o false). 
function unPaso()
	# Se aplica aritmética para convertir el booleano en un número entero. Al multiplicar por 2 y restar 1, se obtiene -1 si rand(Bool) es false y 1 si es true. De esta manera, se generan los valores 1 y -1 con igual probabilidad.
    -(-1)^rand(Bool)
end

# ╔═╡ b73c3530-e0c3-434e-b0e1-5826653e066a
begin
	# Generar un paso aleatorio, que será 1 o -1 con igual probabilidad
	unPaso()
end
...
function variosPasos(n)
	# Genera un arreglo de n elementos, donde cada uno es el resultado de llamar a la función unPaso()
    return [unPaso() for _ in 1:n]
end

# ╔═╡ 99d0b803-e4d7-42a4-9887-c06cf3195ace
# Genera un arreglo con 10 elementos, donde cada uno es un paso aleatorio (1 o -1)
variosPasos(10)
...
function caminataAleatoria(n)
	# Genera un arreglo de n pasos aleatorios (1 o -1)
    pasos = variosPasos(n)
	# Calcula las posiciones acumuladas sumando los pasos
    posiciones = cumsum(pasos)
	# Agrega la posición inicial 0 al inicio del arreglo de posiciones
    return vcat([0], posiciones)
end

# ╔═╡ 45201e31-4a9e-4250-bc5d-27742ed544ef
# Genera un arreglo simulando una caminata aleatoria con 10 pasos
caminataAleatoria(10)
...
function graficaCaminata(caminata)
	# Obtener el número de pasos en la caminata
	num_pasos = length(caminata)
    # Crear un arreglo de posiciones desde 0 hasta el número de pasos
	posiciones = 0:num_pasos

	# Generar una gráfica de dispersión (puntos) con las posiciones y la caminata
    scatter(posiciones, caminata, xlabel = "Número de pasos", ylabel = "Posición",
            title = "Caminata Aleatoria", legend = false)
end

# ╔═╡ c3123c33-7302-4fe7-acc4-6168a102f47a
begin
	caminata = caminataAleatoria(100)  # Ejemplo: Generar una caminata aleatoria de 100 pasos
	graficaCaminata(caminata)  # Generar la gráfica de la caminata aleatoria
end

# ╔═╡ 02861b41-12bd-42a1-b1b1-f4598da5a76d
begin
	n2 = 50  # Ejemplo: Definir el número de pasos para la caminata aleatoria
	graficaCaminata(caminataAleatoria(n2))  # Generar la gráfica de la caminata aleatoria
end
...
# Se crea una funcion con H  que son la cantidad de caminatas aleatorias e I los pasos
function graficaCaminata!(H,I)

	#Se grafica
	plot()

	# El ciclo for se encarga de iterar H veces, donde H representa la cantidad de caminatas aleatorias que se desean generar y graficar. 
	for i in 1:H       

		#Se agrega una línea punteada al gráfico actual. El eje x corresponde a los números de pasos que van desde 0 hasta I, y el eje y corresponde a la caminata aleatoria generada utilizando la función
		plot!(0:I,caminataAleatoria(I),title = "Caminata aleatoria", xlabel = "Número de pasos", ylabel = "Posición", label =false, lef=false) 
	end

	#Muestra la gráfica con las caminatas aleatorias generadas
	plot!()
end

# ╔═╡ 8583ff8a-3431-4a25-85f7-13ae6d9bdd0e
begin
	# Se genera y muestra una gráfica con 5 caminatas aleatorias de 100 pasos cada una.
...
#Anima de 1 hasta pasito	
anim = @animate for i = 1:pasito

#Arreglo del eje x
x1d = collect(1:1:pasito)
#Arreglo dej eje y
y1d=caminataAleatoria(10)

	#Grafica x1d,y1d desde 1 hasta pasito
    plot(x1d[1:i], y1d[1:i],title = "Caminata aleatoria", xlabel = "Número de pasos", ylabel = "Posición", color = "darkblue", label =false)
end
 
gif(anim, "animCaminata1D.gif", fps = 1)
end

# ╔═╡ 40791a97-b6dc-4514-bea2-5d9afd73df9c
#  Genera una animación de una caminata aleatoria en una dimensión con 10 pasos.
animCaminata1D(10)
...
# ╔═╡ 1a9fab8a-cf28-4318-8175-25652cf83334
# SE genera una matriz de tamaño 5x2, donde cada columna representa una dimensión de la caminata 2D y cada fila representa una posición en esa dimensión.
...
function graficaCaminata2D(caminata2D)
    x = caminata2D[:, 1] # Posiciones horizontales
    y = caminata2D[:, 2] # Posiciones verticales

	#Se extraen las posiciones horizontales y verticales de caminata2D utilizando la indexación de matrices. Luego, se utiliza la función plot para generar la gráfica bidimensional.
	
    plot(x, y, linestyle = :dash, linewidth = 2,
         xlabel = "Posición horizontal", ylabel = "Posición vertical",
         title = "Caminata Aleatoria 2D")
end

# ╔═╡ 26caf118-4009-4efe-8d94-43b5a2826040
begin
	#Se genera una gráfica que muestra la trayectoria de una caminata aleatoria en dos dimensiones con n3 + 1 posiciones, incluyendo la posición inicial (0, 0)
	n3 = 500
...
begin
	#La función toma dos argumentos: num_caminatas que representa la cantidad de caminatas aleatorias en dos dimensiones que deseas generar, y num_pasos que indica la cantidad de pasos para cada caminata.
	function graficaCaminata2D!(num_caminatas, num_pasos)
	    plot()

		#El ciclo itera desde i = 1 hasta la cantidad de caminatas aleatorias que se generarán y graficarán.
	    for i in 1:num_caminatas
	        caminata = caminataAleatoria2D(num_pasos)
	        x = caminata[:, 1] # Posiciones horizontales
	        y = caminata[:, 2] # Posiciones verticales
	        
	        plot!(x, y, linestyle = :dash, linewidth = 2, label = false)
	    end
	    
	    xlabel!("Posición horizontal")
	    ylabel!("Posición vertical")
	    title!("Caminatas Aleatorias 2D")
	    plot!()
	end

	# Segeneran caminatas aleatorias en dos dimensiones con 501 posiciones en cada dimensión (incluyendo la posición inicial). Esto significa que cada caminata tendría 501 pasos en total (En este caso).
	num_caminatas = 5
	num_pasos = 500
	graficaCaminata2D!(num_caminatas, num_pasos)
end
...
# Dentro de la función, se utiliza la macro @animate para definir una animación que itera desde 1 hasta pasos. En cada iteración, se generan dos caminatas aleatorias en los ejes x e y, respectivamente.
...
y2d=caminataAleatoria(pasos)    #comienza con la gráfica de y1d
	...
	# Muestra la trayectoria de la caminata aleatoria en dos dimensiones hasta el paso actual. 
    plot(x2d[1:i], y2d[1:i],title = "Caminata aleatoria 2D", xlabel = "Posición x", ylabel = "Posición y", color = "purple", label =false)
end
	gif(anim, "animaCaminata2D.gif", fps = 1)
end

# ╔═╡ c11475df-7eee-4ceb-a887-0cae0e3a16b4
begin
	# Se genera la animación de una caminata aleatoria en dos dimensiones con 10 pasos
	animaCaminata2D(10)
end
...
function graficaCaminata3D(U) #Función caminata 3D
	...
	# Crea un gráfico tridimensional utilizando la función plot. Los argumentos p, q y r representan las posiciones en los ejes x, y, y z
	return plot(p,q,r,title = "Caminata aleatoria 3D", xlabel = "Posición x", ylabel = "Posición y", zlabel = "Posición z", color = "orange
", label =false, aspect_ratio=:equal)       #Devuelve la gráfica de p,q,r
end

# ╔═╡ 640a64c5-a392-447a-b3b5-1fcedc33b558
# Se generará una gráfica tridimensional que muestra la trayectoria de la caminata aleatoria en tres dimensiones con 500 pasos. La posición en cada eje se determinará mediante la función
graficaCaminata3D(500)

# ╔═╡ e51cd406-01c5-4681-8f88-906ff40ebb7e
function graficaCaminata3D!(v,w) #v son la cantidad de caminatas aleatorias y w los pasos
	plot()             #Grafica

	# Itera desde 1 hasta v (cantidad de caminatas)
	for i in 1:v   
		# Agrega una caminata aleatoria en los ejes x, y, y z a la gráfica actual
        # caminataAleatoria(w) genera una caminata aleatoria con w pasos en un eje determinado
...
# ╔═╡ 09635f12-f502-49a1-ad0e-c7532e0b0f3e
# Se genera una gráfica tridimensional que muestra 5 caminatas aleatorias en tres dimensiones, cada una con 100 pasos.
