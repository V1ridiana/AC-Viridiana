...
# ╔═╡ 71107f7c-55e6-40ea-baa8-2fa7857d4488
...
# ╔═╡ 89e3f342-7426-4d84-83c1-036581ae5651
...
$x_{i+1}=x_i-\frac{f(x_i)}{f'(x_1)}$

La fórmula para la aproximación de la raíz en la $(i+1)$-ésima iteración del método de Newton se puede obtener utilizando el mismo razonamiento que se utilizó para obtener la aproximación en la primera iteración. En este caso, consideramos $x_i$ como la aproximación de la raíz después de $i$ iteraciones.

La fórmula para la aproximación de $x_{i+1}$ se obtiene al reemplazar $x_0$ por $x_i$ en la fórmula original:

$$x_{i+1} = x_i - \frac{f(x_i)}{f'(x_i)}.$$

Esta fórmula nos permite calcular sucesivamente las aproximaciones de la raíz en cada iteración del método de Newton. Comenzando con una adivinanza inicial $x_0$, podemos calcular $x_1$, luego usar $x_1$ para calcular $x_2$, y así sucesivamente.
"""
...
# ╔═╡ bd85065e-ca11-4332-8f12-0973e07cdf61
begin
    # Definición de la función newton
	function newton(fa, f_primea, x0a, na)
    x = x0a
   
	# Aplicar la fórmula del método de Newton
	for i in 1:na
        x -= fa(x) / f_primea(x)
    end
	# Devolver la aproximación de la raíz
    return x
end
end

# ╔═╡ 8dd8dee9-8125-4e34-b188-ba1b71fe900e
begin
	# Define la función f y su derivada f_prime
	fa(x) = x^2 - 2
	f_primea(x) = 2x
	
	# Define el valor de la aproximación inicial x0
	x0a = 1.5
	
	# Define el número de iteraciones n
	na = 5
	
	# Llama a la función newton
	resultado = newton(fa, f_primea, x0a, na)
	
	# Imprime el resultado
	println("Aproximación de la raíz después de $na iteraciones: $resultado")
end
...
# ╔═╡ 5c4f2bba-d5a0-4ff3-be5b-7e794f5d81de
# Definición de la función newtonRecursiva
function newtonRecursiva(fb, f_primeb, x, nb)
    if nb == 0
        return x
    else
		# Aplicar la fórmula del método de Newton
		...
		# Llamada recursiva con n-1 iteraciones
		return newtonRecursiva(fb, f_primeb, x, nb - 1)
    end
end

# ╔═╡ 1f3c5b5c-1f14-46c5-a83f-cd73b003f785
begin
	# Definición de la función y su derivada
	fb(x) = x^2 - 2
	f_primeb(x) = 2x
	
	# Aproximación inicial y número de iteraciones
	x0b = 1.5
	nb = 10
	
	# Obtener la aproximación de la raíz utilizando newtonRecursiva
	approx_rootb = newtonRecursiva(fb, f_primeb, x0b, nb)
	println(approx_rootb)
end
...
# Definición de la función newtonDistancia
function newtonDistancia(fc, f_primec, x0c, epsilon)
    # Verificar si el valor de epsilon es válido
    if epsilon <= 0
        println("Error: El valor de epsilon debe ser mayor que cero.")
        return
    end
    ...
    # Iterar hasta que la distancia entre las aproximaciones sea menor a epsilon
    while abs(fc(x_curr)) > epsilon
        # Calcular la siguiente aproximación
        x_prev = x_curr
        x_curr -= fc(x_curr) / f_primec(x_curr)
    end
...
# ╔═╡ 7d7dbc41-0241-419c-8f83-8028654edc88
begin
	# Definición de la función y su derivada
fc(x) = x^2 - 2
f_primec(x) = 2x

# Aproximación inicial y valor de epsilon
x0c = 1.5
epsilon = 0.0001

# Llamada a la función newtonDistancia
resultados = newtonDistancia(fc, f_primec, x0c, epsilon)

# Imprimir el resultado
println("Aproximación de la raíz con una distancia menor a $epsilon: $resultados")
...
# Definición de la función empleando la definición
derivadaNumérica(fd, x, hd) = (fd(x + hd) - fd(x) / hd)

# ╔═╡ 0746c053-de25-4716-9db1-93cfc3e6d354
begin
	# Definición de la función
	fd(x) = x^2
	
	# Punto en el dominio y valor de h
	x = 1.0
	hd = 0.1
	
	# Calcular la aproximación de f'(x)
	approx_derivative = derivadaNumérica(fd, x, hd)
	println(approx_derivative)
end
...
# Definición de la función newtonDN
function newtonDN(fe, x0e, he, ne)
    for i in 1:ne
		# Calcular la derivada numérica utilizando la función derivadaNumérica
        f_primee = derivadaNumérica(fe, x0e, he)
        # Actualizar la aproximación utilizando el método de Newton
		x0e -= fe(x0e) / f_primee
    end
	# Devolver la aproximación de la raíz
    return x0e
...
# ╔═╡ 01dc2b25-9786-4787-9b6c-f772ac023e97
begin
	# Definición de la función
	fe(x) = x^2 - 2
	
	# Aproximación inicial, valor de h y número de iteraciones
	x0e= 1.5
	he = 0.1
	ne = 5
	
	# Obtener la aproximación de la raíz utilizando newtonDN
	approx_roote = newtonDN(fe, x0e, he, ne)
	println(approx_roote)
end
...
# ╔═╡ 2b99525e-8663-412d-8c98-6e1c0e7254ed
md""" Para resolver este problema proponemos los siguientes sliders para crear parámetros interactivos"""

# ╔═╡ ad056a76-689d-49d0-a82f-391a0eb3eb03
deslizadora = @bind l1 Slider(-10:0.1:10, default = 1) #x0

# ╔═╡ 704b8c66-0f20-4233-b232-71cd5c14d877
deslizadorb = @bind l2 Slider(1000:1000:1000000, default=1000) #h

# ╔═╡ 71b82ac3-c3d1-45eb-88fb-2c0e9b81bac2
deslizadorc = @bind l3 Slider(1:1000, default=100) #n

# ╔═╡ 103f3d53-90fa-4dbc-9e81-8433b6f56af1
begin
	# Función f, puedes reasignarla según sea necesario
	fh(x) = cos(x)

	# Implementa el método de Newton utilizando derivadas numéricas. Toma como argumentos fh (la función), l1 (la "adivinanza" inicial), l2 (la aproximación de la derivada numérica) y l3 (el número de iteraciones).
	function NewtonDN(fh, l1, l2, l3)

		#Dentro de un ciclo for, se calcula la derivada numérica de fh en cada iteración y se actualiza el valor de l1 de acuerdo con el método de Newton.
	    for i in 1:l3
	        f_primeh = (fh(x + l2) - fh(l1)) / l2
	        l1 -= fh(l1) / f_primeh
	    end
		# Finalmente, se devuelve el valor aproximado de la raíz.
	    return l1
	end
	
	function plot_function(fh, l1, l2, l3)
		#Se crea una lista de puntos (x, fh(x)) en ese rango utilizando la función range
	    points = [(x, fh(x)) for x in range(l1, length=l3)]
	    plot(points, legend=false, marker=true, color=:gray, title="Gráfica de la función", xlabel="x", ylabel="f(x)")
	end
	# Visualizar los puntos en una gráfica
	plot_function(fh, l1, l2, l3)
...
Si se le asignan funciones distintas de $\cos$ a la variable `fh`, el método de Newton seguirá funcionando y buscará las raíces de la nueva función asignada. El comportamiento y el resultado dependerán de la función específica utilizada.

El método de Newton es una técnica iterativa para encontrar raíces de funciones, y su eficacia puede variar según las propiedades de la función en cuestión. Al asignar una función distinta a `fh`, se aplicará el método de Newton a esa función en lugar de $\cos$. Por lo tanto, el resultado obtenido será la aproximación de la raíz de la nueva función.

Es importante tener en cuenta que el método de Newton puede converger hacia una raíz o divergir dependiendo de la función y el punto de inicio seleccionados. Además, algunas funciones pueden tener múltiples raíces y el método de Newton puede converger a diferentes raíces dependiendo de la condición inicial.

**Sugerencia** Usa la función `newtonDN` y un ciclo `for` junto con la función modificadora `plot!`.
-Cambiar el valor x_0 y observar como cambia, tambien probar con otra funcion a parte de cos.
